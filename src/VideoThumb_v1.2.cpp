// VideoThumb.wlx64 v1.2
// Total Commander video thumbnail WLX plugin.
//
// v1.2 adds:
//   * near-black frame detection with cheap BGRA sampling
//   * automatic retry at 3 s, 5 s and a configurable percentage of duration
//   * best-of-fallback selection if every sampled frame is dark
//
// v1.1 added:
...
struct Settings {
    ...
    // only when the first thumbnail is almost entirely black.
    ...
    std::vector<int> fallback_seek_ms{3000, 5000};
    ...
    Settings() {
        ...
        GetPrivateProfileStringW(L"Thumbnail", L"FallbackSeekMs", L"3000,5000",
...
// Returns the fraction [0..1] of sampled pixels whose luma is at or below the
// configured black threshold. Sampling every fourth pixel in both directions
// makes the test effectively free compared with video decode while remaining
// stable for ordinary TC thumbnail sizes.
double dark_pixel_ratio(const ThumbData& t, int threshold) noexcept {
    if (t.width <= 0 || t.height <= 0 || t.stride < t.width * 4 || t.bgra.empty()) return 0.0;
    ...
    for (int y = 0; y < t.height; y += step) {
        ...
        for (int x = 0; x < t.width; x += step) {
            ...
            // Integer Rec.601-ish luma approximation for BGRA input.
            const int luma = (29 * p[0] + 150 * p[1] + 77 * p[2] + 128) >> 8;
...
std::shared_ptr<ThumbData> decode_with_libav_utf8(const std::string& input, int max_w, int max_h) {
...

    {
        ...
        std::shared_ptr<ThumbData> best_dark;
        double best_dark_ratio = 2.0;
        ...
        auto try_target = [&](int64_t target_us) -> bool {
            ...
            const double ratio = dark_pixel_ratio(*thumb, cfg.black_threshold);
            if (ratio < black_cutoff) {
                result = std::move(thumb);
                return true;
            }

            // If every sampled position is dark, return the least-dark one rather
            // than blindly returning the first completely black frame.
            if (!best_dark || ratio < best_dark_ratio) {
                best_dark = std::move(thumb);
        ...
        if (!result && best_dark) result = std::move(best_dark);
    }
