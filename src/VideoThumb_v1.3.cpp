...
// Cheap thumbnail statistics used to reject black frames and black title cards.
// Sampling every fourth pixel in both directions examines only ~1/16 of the
// thumbnail. This is negligible compared with video seek/decode cost.
struct FrameStats {
...
FrameStats frame_stats(const ThumbData& t, int threshold) noexcept {
    FrameStats out{};
    if (t.width <= 0 || t.height <= 0 || t.stride < t.width * 4 || t.bgra.empty()) return out;

    uint64_t dark = 0, mid = 0, bright = 0, total = 0, luma_sum = 0;
...
bool suspicious_frame(const FrameStats& st, const Settings& cfg) noexcept {
    ...
    if (!cfg.title_card_detection) return false;
    ...
    return st.dark_ratio >= title_dark &&
           st.mid_ratio <= title_mid &&
           st.mean_luma <= static_cast<double>(cfg.title_card_mean_luma_max);
}

// Higher is better. This is used only if every sampled frame is suspicious.
// It strongly rewards occupied/non-black image area, then midtone detail and
// average brightness. A black screen or sparse title card therefore loses to
// an ordinary scene without doing expensive image analysis.
double frame_quality_score(const FrameStats& st) noexcept {
    return (1.0 - st.dark_ratio) * 1000.0 +
           st.mid_ratio * 180.0 +
           st.mean_luma * 0.35;
}
...
std::shared_ptr<ThumbData> decode_with_libav_utf8(const std::string& input, int max_w, int max_h) {
...

    {
        ...
        auto try_target = [&](int64_t target_us) -> bool {
            ...
            const FrameStats stats = frame_stats(*thumb, cfg.black_threshold);
            ...
            // candidate instead of returning a black frame or sparse title card.
